...
```
**转换之后的标签格式说明**
模型最终训练只是里面的一部分内容，movingObjects字段下的就是每一个目标的属性，但是训练的时候只需要三维框（cuboid3D）的尺寸（cuboidExtent）、中心点坐标（position）、和角度（rorientation的第三个元素）, 类别（objectType）. 原始数据标签type有如很多类，我们需要对应到如下几类, 'Pedestrian', 'Mbike', 'Car', 'Bus', 'Tricycle'，分别对应0-4. 所以你在处理数据的时候需要从中提取这些数据，保存为.txt文本。
  ┌─────────────┬─────────────────────┬─────────────────────┬─────────────────┐
  │  模型输出 ID │  json中的名称         │   生成的txt中的名称   │                 │
  ├─────────────┼─────────────────────┼─────────────────────┼─────────────────┤
  │      0      │    Pedestrian       │  Pedestrian         │      行人       │
  ├─────────────┼─────────────────────┼─────────────────────┼─────────────────┤
  │      1      │          Bicycle    │         Mbike       │      摩托车/骑行者│
  ├─────────────┼─────────────────────┼─────────────────────┼─────────────────┤
  │      2      │          Car        │     Car             │      车辆       │
  ├─────────────┼─────────────────────┼─────────────────────┼─────────────────┤
  │      3      │    Heavy Truck      │    Bus              │       大车       │
  ├─────────────┼─────────────────────┼─────────────────────┼─────────────────┤
  │      4      │          Bus        │   Bus               │      三轮车      │
  └─────────────┴─────────────────────┴─────────────────────┴─────────────────┘
- **格式**：纯文本 (.txt)，每行一个目标
